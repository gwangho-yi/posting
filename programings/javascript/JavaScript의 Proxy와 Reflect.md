...
자바도 존재하고 자바스크립트에도 존재하는 Proxy는 언어 동작 레벨에서 자바와는 어떤 차이로 동작하는지 궁금해서 알아봤다. 자바 진영에서는 흔히 스프링 AOP나 Jpa를 배울 때 Proxy를 접했는데 최근에 프론트를 자주 만지는 상황이라 이번 기회에 JS도 한 번 파고 들어서 깊이를 더해보고자 한다.
