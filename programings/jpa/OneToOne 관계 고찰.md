...
4. 연관관계를 끊고 ID만 저장하는 방식
JPA 연관관계 매핑 자체를 포기하고, 대상 테이블에 주 테이블의 ID를 일반 필드로 저장하는 방식이다. Aggregate 간 참조는 ID로만 한다는 DDD 원칙(Vaughn Vernon의 IDDD에서 권장)과도 일치하며, 가장 단순하고 깔끔한 방식이기도 하다.

```kotlin
@Entity
@Table(name = "member_case4")
class Member(
    @Id @GeneratedValue(strategy = GenerationType.IDENTITY)
    val id: Long? = null,
...
class Locker(
    ...
    val memberId: Long  // FK를 일반 필드로 관리
)
```

Locker 조회가 필요하면 Repository에서 직접 쿼리하면 된다.
```kotlin
interface LockerRepository : JpaRepository<Locker, Long> {
    fun findByMemberId(memberId: Long): Locker?
}
```

이 방식의 장점은 다음과 같다.
- Lazy Loading 문제가 원천적으로 없다 (연관관계 매핑 자체가 없으므로)
- DB 확장성 확보: 1:N 전환 시 UNIQUE 제약조건만 제거하면 된다
- 엔티티 간 결합도가 낮아 Aggregate 경계가 명확해진다

다만 Cascade, orphanRemoval 같은 JPA 생명주기 관리를 직접 처리해야 하고, Join이 필요하면 JPQL이나 QueryDSL에서 명시적으로 작성해야 한다는 트레이드오프는 있다.