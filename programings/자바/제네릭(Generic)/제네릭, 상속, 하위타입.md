특정 변수의 타입이 다른 타입에 호환된다면 할당이 가능하다.
```java
...
`boxTest`는 `Box<Integer>` , `Box<Double>`을 인자로 받을 수 있을까? 정답은 **"No"** 다 왜냐하면, 둘 다 `Box<Number>` 의 하위타입(Sub Type)이 아니기 때문이다. 제네릭을 이해하는데 있어 쉽게 오해할 수 있는 부분이지만, 반드시 학습해야 할 중요한 개념이다.
...
```
타입 파라메터 `E`에 해당하는 `Interger` 가 그대로 전이 된다. `ArrayList<String>`과 같은 선언은 허용되지 않는다.
정리하자면 아래와 같다.
